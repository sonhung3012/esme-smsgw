...
public class EsmeMessageDaoImpl extends GenericDaoSpringHibernateTemplate<EsmeMessage, Long> implements EsmeMessageDao {
	...
	public List<EsmeMessage> findAll(EsmeMessage esmeMessage, int firstItemIndex, int maxItems) throws Exception {

		return findAll(esmeMessage, firstItemIndex, maxItems, false);
	}

	private Criteria createCriteria(EsmeMessage esmeMessage, String orderedColumn, boolean asc, boolean exactMatch) throws Exception {
			...
			if (!FieldChecker.isEmptyString(description)) {
				...
				if (checkStartsWith != null) {
					...
					if (exactMatch) {
						...
						or.add(Restrictions.like("description", "%" + description + "%").ignoreCase());
					}
				}
			}
		...
		if (orderedColumn != null && FieldChecker.classContainsField(EsmeMessage.class, orderedColumn)) {
			if (asc) {
	...
	@Override
	public List<EsmeMessage> findAll(EsmeMessage esmeMessage, boolean exactMatch) throws Exception {
	...
	@Override
	public List<EsmeMessage> findAll(EsmeMessage esmeMessage, int firstItemIndex, int maxItems, boolean exactMatch) throws Exception {

		return findAll(esmeMessage, null, false, firstItemIndex, maxItems, exactMatch);
	}

	@Override
	public int count(EsmeMessage esmeMessage, boolean exactMatch) throws Exception {
	...
	@Override
	public List<EsmeMessage> findAll(EsmeMessage esmeMessage, String sortedColumn, boolean ascSorted, int firstItemIndex, int maxItems, boolean exactMatch) throws Exception {
	...
	public int checkExited(EsmeMessage esmeMessage) throws Exception {
		...
		if (esmeMessage.getCode() != null) {

			criteria.add(Expression.eq("code", esmeMessage.getCode()));
		} else if (esmeMessage.getName() != null) {

			criteria.add(Expression.eq("name", esmeMessage.getName()));
		}
