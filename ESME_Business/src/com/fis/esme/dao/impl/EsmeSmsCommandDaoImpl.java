...
public class EsmeSmsCommandDaoImpl extends
		GenericDaoSpringHibernateTemplate<EsmeSmsCommand, Long> implements
		EsmeSmsCommandDao {
	...
	private Criteria createCriteria(EsmeSmsCommand smsCommand,
		...
		if (orderedColumn != null
				&& FieldChecker.classContainsField(EsmeSmsCommand.class,
						orderedColumn)) {
			if (asc) {
	...
	public List<EsmeSmsCommand> findAll(EsmeSmsCommand smsCommand, int firstItemIndex,
		...
		return findAll(smsCommand, null, false, firstItemIndex, maxItems,
				exactMatch);
	}
	...
	public List<EsmeSmsCommand> findAll(EsmeSmsCommand smsCommand, String sortedColumn,
		...
		Criteria finder = createCriteria(smsCommand, sortedColumn, ascSorted,
				exactMatch);
	...
	public boolean checkConstraints(Long id) throws Exception {
		...
		for (Class c : cls) {
			...
			criteria.add(Expression.eq("esmeSmsComamnd", obj));
			criteria.setProjection(Projections.count("esmeSmsComamnd"));
