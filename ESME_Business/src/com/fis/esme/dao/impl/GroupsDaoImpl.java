...
public class GroupsDaoImpl extends
		GenericDaoSpringHibernateTemplate<Groups, Long> implements GroupsDao {
	...
	public List<Groups> findAll(Groups Groups, int firstItemIndex, int maxItems)
			throws Exception {
		return findAll(Groups, firstItemIndex, maxItems, false);
	}

	private Criteria createCriteria(Groups Groups, String orderedColumn,
		...
		if (Groups != null) {
			...
			if (!FieldChecker.isEmptyString(description)) {
				String checkStartsWith = BusinessUtil
						.checkStartsWith(description);
				if (checkStartsWith != null) {
					or.add(Expression.like("desciption", checkStartsWith,
							MatchMode.START).ignoreCase());
				} else {
					if (exactMatch) {
						or.add(Restrictions.eq("desciption", description)
								.ignoreCase());
					} else {
						or.add(Restrictions.like("desciption",
		...
		if (orderedColumn != null
				&& FieldChecker.classContainsField(Groups.class, orderedColumn)) {
			if (asc) {
	...
	public List<Groups> findAll(Groups Groups, int firstItemIndex,
		...
		return findAll(Groups, null, false, firstItemIndex, maxItems,
				exactMatch);
	}
	...
	public List<Groups> findAll(Groups Groups, String sortedColumn,
		...
		Criteria finder = createCriteria(Groups, sortedColumn, ascSorted,
				exactMatch);
	...
	public Groups findBySubGgroup(long subId) throws Exception {
		String strSQL = "select * from GROUPS where GROUP_ID in (select GROUP_ID from"
				+ " SUB_GROUP where sub_id=" + subId + ") and status='1'";
