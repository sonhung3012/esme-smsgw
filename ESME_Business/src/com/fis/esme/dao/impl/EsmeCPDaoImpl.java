...
public class EsmeCPDaoImpl extends
		GenericDaoSpringHibernateTemplate<EsmeCp, Long> implements
		EsmeCPDao {
	...
	private Criteria createCriteria(EsmeCp esmeCp,
		...
		if (esmeCp != null) {
			...
			if (!FieldChecker.isEmptyString(username)) {
				...
				if (checkStartsWith != null) {
					or.add(Expression.like("username", checkStartsWith,
							MatchMode.START).ignoreCase());
				} else {
					if (exactMatch) {
						or.add(Restrictions.eq("username", username).ignoreCase());
					} else {
						or.add(Restrictions.like("username", "%" + username + "%")
								.ignoreCase());
					}
				}
			}
			...
			if (!FieldChecker.isEmptyString(shortCode)) {
				...
				if (checkStartsWith != null) {
					or.add(Expression.like("defaultShortCode", checkStartsWith,
							MatchMode.START).ignoreCase());
				} else {
					if (!exactMatch) {
						or.add(Restrictions.eq("defaultShortCode", shortCode).ignoreCase());
					} else {
						or.add(Restrictions.like("defaultShortCode", "%" + shortCode + "%")
								.ignoreCase());
					}
				}
			}
		...
		if (orderedColumn != null
				&& FieldChecker.classContainsField(EsmeCp.class,
						orderedColumn)) {
			if (asc) {
	...
	public List<EsmeCp> findAll(EsmeCp esmeCp, int firstItemIndex,
		...
		return findAll(esmeCp, null, false, firstItemIndex, maxItems,
				exactMatch);
	}
	...
	public List<EsmeCp> findAll(EsmeCp esmeCp, String sortedColumn,
		...
		Criteria finder = createCriteria(esmeCp, sortedColumn, ascSorted,
				exactMatch);
