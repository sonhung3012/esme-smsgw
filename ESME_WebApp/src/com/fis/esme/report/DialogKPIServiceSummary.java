...
import com.fis.esme.client.ServiceSummaryTransfererClient;
...
public class DialogKPIServiceSummary extends Window implements Action.Handler,
	...
	public DialogKPIServiceSummary() throws Exception {
		super(TM.get(DialogKPIServiceSummary.class.getName()));
		LogUtil.logAccess(DialogKPIServiceSummary.class.getName());
		this.setWidth("400px");
	...
	private void init() throws Exception {
		...
		Object[] visibleProperties = { "fromDate", "toDate", "service" };
	...
	private void onAccept() {
		...
		try {
			if (formIsValid()) {
				...
				ServiceSummaryTransferer smsService = ServiceSummaryTransfererClient
						.getService();
				...
				columns.add(column1);
				columns.add(column2);
				List<EsmeDailyServiceSummary> list = smsService
						.findAllByDateAndId(searcher.getService()
								.getServiceId(), searcher.getFromDate(),
								searcher.getToDate());
				...
				int stt = 1, stt1 = 1;
				for (EsmeDailyServiceSummary smsLog : list) {

					obj1 = new Vector<Object>();
					obj1.add(stt);
					obj1.add((smsLog.getSummaryDate() != null) ? FormUtil.simpleShortDateFormat
							.format(smsLog.getSummaryDate()) : "");
					obj1.add((searcher.getService().getName() != null) ? searcher
							.getService().getName() : "");
					// obj1.add((smsLog.getServiceId() != null) ? smsLog
					// .getServiceId() : "");
					...
					obj2.add((smsLog.getSummaryDate() != null) ? FormUtil.simpleShortDateFormat
							.format(smsLog.getSummaryDate()) : "");
					obj2.add((searcher.getService().getName() != null) ? searcher
							.getService().getName() : "");
//					obj2.add((smsLog.getServiceId() != null) ? smsLog
//							.getServiceId() : "");
				...
				Object rs = MultiReportUtil.doExportData(absolutePath,
						"ServiceSummary", null, parameters, vData, columns,
	...
	private class searchFormFieldFactory extends DefaultFieldFactory {
		...
		private ComboBox cbbService = new ComboBox("Service");
		...
		private void initComponent() throws Exception {
			...
			cbbService.setRequiredError(TM.get(
					"common.field.msg.validator_nulloremty",
					cbbService.getCaption()));
			cbbService.setContainerDataSource(serviceData);
			cbbService.setInputPrompt(TM.get(
					"common.field_combobox.inputprompt", serviceLb.getValue()
	...
	protected class Searcher {
		...
		private Date toDate;
		private EsmeServices service;

		public Searcher(Date fromDate, Date toDate, EsmeServices service) {
		...
		public EsmeServices getService() {
			return service;
		}

		public void setService(EsmeServices service) {
			this.service = service;
		}
