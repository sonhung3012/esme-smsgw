...
import com.fis.esme.client.CpSummaryTransfererClient;
import com.fis.esme.component.ConfirmDeletionDialog;
import com.fis.esme.cp.Exception_Exception;
...
public class DialogKPICpSummary extends Window implements Action.Handler,
	...
	public DialogKPICpSummary() {
		super(TM.get(DialogKPICpSummary.class.getName()));
		LogUtil.logAccess(DialogKPICpSummary.class.getName());
		this.setWidth("400px");
	...
	private void init() {
		...
		Object[] visibleProperties = { "fromDate", "toDate","cp" };
	...
	private void onAccept() {
		...
		try {
			if (formIsValid()) {
				...
				CpSummaryTransferer smsService = CpSummaryTransfererClient
						.getService();
				...
				columns.add(column1);
				columns.add(column2);
				List<EsmeDailyCpSummary> list = smsService
						.findAllByDateAndId(searcher.getCp()
								.getCpId(), searcher.getFromDate(),
								searcher.getToDate());
...
//							.getCpId() : "");
					...
					obj2.add((smsLog.getSummaryDate() != null) ? FormUtil.simpleShortDateFormat
							.format(smsLog.getSummaryDate()) : "");
					obj2.add((searcher.getCp().getCode() != null) ? searcher
							.getCp().getCode() : "");
//					obj2.add((smsLog.getCpId() != null) ? smsLog
//							.getCpId() : "");
				...
				Object rs = MultiReportUtil.doExportData(absolutePath,
						"CPSummary", null, parameters, vData, columns,
	...
	private class searchFormFieldFactory extends DefaultFieldFactory {
		...
		private ComboBox cbbCP = new ComboBox("CP");
		private BeanItemContainer<EsmeCp> cpData = new BeanItemContainer<EsmeCp>(
				EsmeCp.class);
		...
		private void initComponent() {
			...
			try {
				cpData.addAll(CacheServiceClient.serviceCp.findAllWithoutParameter());
			...
			cbbCP.setRequiredError(TM.get(
					"common.field.msg.validator_nulloremty",
					cbbCP.getCaption()));
			cbbCP.setContainerDataSource(cpData);
			cbbCP.setInputPrompt(TM.get(
					"common.field_combobox.inputprompt", cbbCP.getCaption()
	...
	protected class Searcher {
		...
		public Searcher(Date fromDate, Date toDate, EsmeCp cp) {
			super();
