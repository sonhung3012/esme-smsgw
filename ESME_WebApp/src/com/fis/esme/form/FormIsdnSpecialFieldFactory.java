...
public class FormIsdnSpecialFieldFactory extends DefaultFieldFactory implements
		PropertyExisted {
	private final TextField txtMsisdn = new TextField(
			TM.get("special.field.msisdn.caption"));
	private final TextField txtName = new TextField(
			TM.get("special.field.name.caption"));
	private final ComboBox cbbType = new ComboBox(
			TM.get("special.field.type.caption"));
	private final ComboBox cbbReason = new ComboBox(
			TM.get("special.field.reason.caption"));
	private final ComboBox cbbStatus = new ComboBox(
			TM.get("special.field.status.caption"));
	...
	private void initComboBox() {
		...
		cbbStatus.setItemCaption(strActive,
				TM.get("special.status.value." + strActive));
		cbbStatus.setItemCaption(strInactive,
				TM.get("special.status.value." + strInactive));
		...
		cbbType.setItemCaption(strType1,
				TM.get("special.type.value." + strType1));
		cbbType.setItemCaption(strType2,
				TM.get("special.type.value." + strType2));
		cbbType.setNullSelectionAllowed(false);
		cbbType.setRequired(true);
		cbbType.setRequiredError(TM.get(
				"common.field.msg.validator_nulloremty", cbbType.getCaption()));
		...
		cbbReason.setItemCaption(strReason1,
				TM.get("special.reason.value." + strReason1));
		cbbReason.setItemCaption(strReason2,
				TM.get("special.reason.value." + strReason2));
		cbbReason.setItemCaption(strReason3,
				TM.get("special.reason.value." + strReason3));
		cbbReason.setItemCaption(strReason4,
				TM.get("special.reason.value." + strReason4));
		cbbReason.setNullSelectionAllowed(false);
		cbbReason.setRequired(true);
		cbbReason
				.setRequiredError(TM.get(
						"common.field.msg.validator_nulloremty",
						cbbReason.getCaption()));
	...
	private void initTextField() {
		...
		String errorCodeMsg = TM.get("common.field_msisdn.msg.validator",
				txtMsisdn.getCaption());
		txtMsisdn.addValidator(new CustomRegexpValidator(TM.get(
				"special.field_msisdn.regexp.validator_error",
		...
		PropertyExistedValidator fieldExistedValicator = new PropertyExistedValidator(
				TM.get("common.field.msg.validator_existed",
						txtMsisdn.getCaption()), this, "msisdn");
		...
		nullNameMsg = TM.get("common.field.msg.validator_nulloremty",
				txtName.getCaption());
	...
	public boolean isPropertyExisted(String property, Object value) {
		...
		if (property.equals("msisdn")) {
			if (value.toString().equalsIgnoreCase(oldMsisdn)) {
				...
				if (CacheServiceClient.serviceIsdnSpecial.checkExisted(null,
						obj) >= 1) {
					return false;
				} else {
					return true;
