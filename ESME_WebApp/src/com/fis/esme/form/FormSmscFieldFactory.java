...
public class FormSmscFieldFactory extends DefaultFieldFactory implements
		PropertyExisted {
	private final TextField txtCode = new TextField(
			TM.get("smsc.field_code.caption"));
	private final TextField txtName = new TextField(
			TM.get("smsc.field_name.caption"));
	private final TextArea txtDescription = new TextArea(
			TM.get("smsc.field_description.caption"));
	private final ComboBox cbbStatus = new ComboBox(
			TM.get("smsc.field_status.caption"));
	private final ComboBox cbbStartupType = new ComboBox(
			TM.get("smsc.field_startuptype.caption"));
	private final TextField txtClassName = new TextField(
			TM.get("smsc.field_classname.caption"));
	private final TextField txtDefaultShortCode = new TextField(
			TM.get("smsc.field_defaultshortcode.caption"));
...
//		cbbStatus.setInputPrompt(TM.get("cbb.inputprompt"));
		...
		cbbStatus.setItemCaption(strActive,
				TM.get("smsc.field_combobox.value_active"));
		cbbStatus.setItemCaption(strInactive,
				TM.get("smsc.field_combobox.value_inactive"));
		...
		cbbStartupType.setItemCaption(startupType_val0,
				TM.get("smsc.form.startuptype_0"));
		cbbStartupType.setItemCaption(startupType_val1,
				TM.get("smsc.form.startuptype_1"));
		cbbStartupType.setItemCaption(startupType_val2,
				TM.get("smsc.form.startuptype_2"));
...
//		txtName.setInputPrompt(TM.get("smsc.name.default"));
		...
		fieldExistedValicator = new PropertyExistedValidator(
				TM.get("common.field.msg.validator_existed",
...
//				.get("smsc.default_shortcode.default"));
		...
		txtDefaultShortCode.setRequired(true);
		String nullScMsg = TM.get("common.field.msg.validator_nulloremty",
				txtDefaultShortCode.getCaption());
		...
		String sms = TM.get("smsc.field_short_code.valid",
				txtDefaultShortCode.getCaption());
		txtDefaultShortCode.addValidator(new CustomRegexpValidator("[0-9]*",
				sms));
...
//		txtClassName.setInputPrompt(TM.get("smsc.class_name.default"));
		...
		String nullClnMsg = TM.get("common.field.msg.validator_nulloremty",
				txtClassName.getCaption());
