...
public class FormIsdnPrefixFieldFactory extends DefaultFieldFactory implements
		PropertyExisted {
	private final TextField txtPrefixValue = new TextField(
			TM.get("isdnprefix.field_prefix_value.caption"));
	private final ComboBox cbbStatus = new ComboBox(
			TM.get("isdnprefix.field_status.caption"));
	...
	private void initComboBox() {
		...
		cbbStatus.setItemCaption(strActive,
				TM.get("isdnprefix.field_combobox.value_active"));
		cbbStatus.setItemCaption(strInactive,
				TM.get("isdnprefix.field_combobox.value_inactive"));
	...
	private void initTextField() {
		...
		PropertyExistedValidator fieldExistedValicator = new PropertyExistedValidator(
				TM.get("common.field.msg.validator_existed",
						txtPrefixValue.getCaption()), this, "prefixValue");
		txtPrefixValue.addValidator(fieldExistedValicator);
		String sms = TM.get("isdnprefix.field_prefix_value.valid",
				txtPrefixValue.getCaption());
		txtPrefixValue.addValidator(new CustomRegexpValidator("[0-9\\?\\*]*",
				sms));
