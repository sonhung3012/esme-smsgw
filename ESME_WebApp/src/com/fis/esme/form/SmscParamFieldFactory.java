...
public class SmscParamFieldFactory extends DefaultFieldFactory implements
	...
	private final TextField txtName = new TextField(
			TM.get("smscParam.field.name"));
	private final TextField txtValue = new TextField(
			TM.get("smscParam.field.value"));
	...
	private BeanItemContainer<EsmeSmsc> beanAction = new BeanItemContainer<EsmeSmsc>(
			EsmeSmsc.class);
	...
	private void initComboBox() throws Exception {
		...
		List<EsmeSmsc> lstSmsc = CacheServiceClient.smscService
				.findAllWithOrderPaging(esmeSmsc, null, false, -1, -1, true);
...
//				TM.get("frmActionparam.action2")));
	...
	public void insertItemTempForCombobox(EsmeSmscParam actionParam) {
		if (actParam != null) {
			if (actParam.getSmscId() != null
					&& cbSmsc.getItem(actParam.getSmscId()) != null) {
