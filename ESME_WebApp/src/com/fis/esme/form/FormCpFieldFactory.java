...
public class FormCpFieldFactory extends DefaultFieldFactory implements
		PropertyExisted, NameExisted, FieldsValidatorInterface {
	private final TextField txtCode = new TextField(
			TM.get("cp.field_code.caption"));
	private final TextField txtUserName = new TextField(
			TM.get("cp.field_username.caption"));
	private final TextField txtPass = new TextField(
			TM.get("cp.field_pass.caption"));
	private final ComboBox cbbProtocal = new ComboBox(
			TM.get("cp.field_protocal.caption"));
	private final TextField txtshortcode = new TextField(
			TM.get("cp.field_shortcode.caption"));
	private final TextField txtReceivePassword = new TextField(
			TM.get("cp.field_ReceivePassword.caption"));
	private final TextField txtReceiveUsername = new TextField(
			TM.get("cp.field_ReceiveUsername.caption"));
	private final TextField txtReceiveUrlMsg = new TextField(
			TM.get("cp.field_ReceiveUrlMsg.caption"));
	private final TextArea txtDescription = new TextArea(
			TM.get("cp.field_description.caption"));
	private final ComboBox cbbStatus = new ComboBox(
			TM.get("cp.field_status.caption"));
	...
	private void initComboBox() {
		...
		cbbProtocal.setItemCaption(strSmtp,
				TM.get("cp.field.combobox.protocal.smtp"));
		cbbProtocal.setItemCaption(strHttp,
				TM.get("cp.field.combobox.protocal.http"));
		cbbProtocal.setNullSelectionAllowed(false);
		cbbProtocal.setRequired(true);
		cbbProtocal.setRequiredError(TM.get(
				"common.field.msg.validator_nulloremty",
				cbbProtocal.getCaption()));
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if (cbbProtocal.getValue() != null) {
					...
					if (cbbProtocal.getValue().toString().equalsIgnoreCase("2")) {
						...
						txtReceiveUrlMsg.setRequiredError(TM.get(
								"common.field.msg.validator_nulloremty",
								txtReceiveUrlMsg.getCaption()));
					} else {
		...
		cbbStatus.setItemCaption(strActive,
				TM.get("cp.field_combobox.value_active"));
		cbbStatus.setItemCaption(strInactive,
				TM.get("cp.field_combobox.value_inactive"));
	...
	private void initTextField() {
		...
		txtCode.addValidator(new CustomRegexpValidator(TM.get(
				"cp.field_code.regexp.validator_error", txtCode.getCaption()),
				true, errorCodeMsg, true));
		...
		String nullNameMsg12 = TM.get("common.field.msg.validator_nulloremty",
				txtUserName.getCaption());
		...
		FieldsValidator fieldValicator = new FieldsValidator(this, "username",
				new Field[] { txtUserName });
		...
		String nullNameMsg9 = TM.get("common.field.msg.validator_nulloremty",
		...
		String errorCodeMsg1 = TM.get("common.field.msg.validator_isint",
				txtshortcode.getCaption());
		txtshortcode.addValidator(new CustomRegexpValidator(TM.get(
				"cp.field_shortcode.byte.regexp.validator_error",
				txtshortcode.getCaption()), true, errorCodeMsg1, true));
		codeExistedValidator = new NameExistedValidator(TM.get(
				"common.field.msg.validator_existed",
				TM.get("defaultshortcode.exit")), this);
		txtshortcode.addValidator(codeExistedValidator);

		txtReceivePassword.setMaxLength(100);
		txtReceivePassword.setSecret(true);
		String nullNameMsgrp = TM.get("common.field.msg.validator_nulloremty",
				txtReceivePassword.getCaption());
		...
		txtReceivePassword.setSecret(true);
		String nullNameMsgru = TM.get("common.field.msg.validator_nulloremty",
				txtReceivePassword.getCaption());
		...
		txtReceivePassword.setSecret(true);
		String nullNameMsg5 = TM.get("common.field.msg.validator_nulloremty",
				txtReceivePassword.getCaption());
		...
		String nullNameMsg13 = TM.get("common.field.msg.validator_nulloremty",
				txtPass.getCaption());
	...
	public boolean isNameExisted(String name) {
		...
		if (trim.equalsIgnoreCase(oldShortCode) || "".equals(trim)) {
			...
			try {
				...
				ArrayList<EsmeCp> cpArr = (ArrayList<EsmeCp>) serviceCp
						.findAllWithOrderPaging(cp, "", false, 0, 10, false);
				if (cpArr.size() >= 1) {
					return false;
				} else {
					return true;
	...
	public Object isValid(String property, Object currentFieldValue,
		...
		if (oldUsername != null && (trim.equalsIgnoreCase(oldUsername.toUpperCase()) || "".equals(trim))) {
			...
			try {
				...
				ArrayList<EsmeCp> cpArr = (ArrayList<EsmeCp>) serviceCp
						.findAllWithOrderPaging(esmecp, "", false, 0, 10, true);
				if (cpArr.size() >= 1) {
