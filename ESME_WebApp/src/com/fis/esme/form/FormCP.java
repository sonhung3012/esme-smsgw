...
public class FormCP extends VerticalLayout implements PanelActionProvider,
	...
	private void initTable() {
		...
		tbl = new CustomTable("", data, pnlAction) {
			...
			protected String formatPropertyValue(Object rowId, Object colId,
				...
				if ("protocal".equals(pid)) {
					...
					} else if (property.getValue().toString()
							.equalsIgnoreCase("2")) {
		...
		pnlAction.setValueForCboField(
				TM.get("cp.table.filteredcolumns").split(","),
				TM.get("cp.table.filteredcolumnscaption").split(","));
	...
	private void displayData(String sortedColumn, boolean asc, int start,
		...
		try {
			data.removeAllItems();
			data.addAll(serviceCP.findAllWithOrderPaging(skSearch,
	...
	private Window createDialog(Item item) {
		...
		form.setVisibleItemProperties(TM.get("cp.form.visibleproperties")
				.split(","));
	...
	public void showDialog(Object object) {
		...
		if (action == PanelActionProvider.ACTION_EDIT) {
			...
			fieldFactory.setOldShortCode(((EsmeCp) object)
					.getDefaultShortCode());
	...
	public void accept() {
		try {
			...
			String strPassword = (msv.getPassword() != null) ? StringUtil
					.encrypt(msv.getPassword(), StringUtil.nvl(SessionData
							.getAppClient()
							.getSessionValue("Encrypt.Algorithm"), "")) : null;

					
			if (pnlAction.getAction() == PanelActionProvider.ACTION_EDIT
					&& msv.getPassword().equalsIgnoreCase(fieldFactory
							.getOldPassWord())) {
			...
			/*
			 * System.out.println("ss>>>>>>" + msv.getReceivePassword()); if
			 * (msv.getReceivePassword() != null &&
			 * !msv.getReceivePassword().equalsIgnoreCase(null) &&
			 * !msv.getReceivePassword().equalsIgnoreCase("")) { String
			 * strRePassword = (msv.getReceivePassword() != null) ? StringUtil
			 * .encrypt(msv.getReceivePassword(), StringUtil.nvl(
			 * SessionData.getAppClient().getSessionValue( "Encrypt.Algorithm"),
			 * "")) : null; msv.setReceivePassword(strRePassword); }
			 */
			...
			if (pnlAction.getAction() == PanelActionProvider.ACTION_ADD
				...
				try {
					...
					if (id > 0) {
						...
						container.initPager(serviceCP.count(null,
								DEFAULT_EXACT_MATCH));

						// tbl.addItem(msv);
						tblSetARowSelect(msv);

						LogUtil.logActionInsert(FormCP.class.getName(), "ESME_CP", "CP_ID",
								"" + msv.getCpId() + "", null);
				...
				try {
					...
					Vector v = LogUtil.logActionBeforeUpdate(FormCP.class.getName(),
							"ESME_CP", "CP_ID", "" + msv.getCpId() + "", null);
	...
	private void doDelete() {
		...
		for (EsmeCp msv : canDelete) {
			try {
				LogUtil.logActionDelete(FormCP.class.getName(), "ESME_CP", "CP_ID",
						"" + msv.getCpId() + "", null);
