...
public class FormMessageSchedulerApprover extends VerticalLayout implements
	...
	private void initLayout() {
		...
		pnlAction.setFromCaption(TM.get(FormMessageSchedulerApprover.class
				.getName()));
		...
		btnDate.addListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if (cboType.getValue() != null
					...
					weeklyTable = new DialogWeeklyTableApprover("Scheduler",
							cboType.getValue().toString(),
							FormMessageSchedulerApprover.this);
	...
	private void initTable() {
		...
		tbl.setVisibleColumns(TM
				.get("messageschedulerapprover.table.setvisiblecolumns").split(","));
		tbl.setColumnHeaders(TM.get("messagescheduler.table.setcolumnheaders")
				.split(","));
		tbl.setStyleName("commont_table_noborderLR");

		String[] columnWidth = TM.get(
				"messagescheduler.table.columnwidth").split(",");
		String[] columnWidthValue = TM.get(
				"messageschedulerapprover.table.columnwidth").split(",");
		...
		container = new TableContainerApprover(tbl, this, Integer.parseInt(TM
				...
				try {
					...
					if (messId > 0) {
						...
						LogUtil.logActionInsert(
								FormMessageSchedulerApprover.class.getName(),
	...
	public void onApprover() {
		...
		try {
			for (int i = 0; i < list.size(); i++) {
				...
				EsmeMessage rowEsmeMessage = rowEsmeMessageContent
						.getEsmeMessage();
				...
				tbl.refreshRowCache();
				MessageAlerter.showMessageI18n(getWindow(),
						TM.get("messagescheduler.approver.true"));
			}
