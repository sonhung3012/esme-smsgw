...
public class CustomLogin_Admin extends VerticalLayout
{
	...
	public CustomLogin_Admin(MainApplication app)
	{
...
//		custom.addComponent(language, "changelanguage");
	...
	public boolean login(final String strUserName, String strPassword)
	{
		try
		{
			...
			request.setString("strIPAddress", SessionData.getAppClient().getIpRemoteAddr());
			request.setString("strUserName", strUserName);
			request.setString("strPassword", strPassword);
			DDTP response = SessionData.getAppClient().sendRequest("PermissionBean",
					"login", request);
...
//				final FormChangePassword changePassword = new FormChangePassword();
...
//				// throw new AppException("FSS-10003", "CustomLogin");
			...
			LogUtil.logAccess("com.fss.admin.form.DialogLogin");
			...
			//
			// SessionData.getAppClient().setSessionValue("SessionUser", strUserName);
