...
public class SmsBeanOracleEx extends SmsBean {

	public SmsBeanOracleEx() {
	...
	public int updateSMSMORetry(Connection mcnMain, String status, String mo_id) {
		...
		try {
			String strUpdateSMSMOSRetry = " update esme_sms_mo set status = ?,RETRY_NUMBER = nvl(RETRY_NUMBER,0)+1, last_update=sysdate where mo_id = ?  ";
	...
	public int updateSMSMOStatus(Connection mcnMain, String status, String mo_id) {
		...
		try {
			// update mo status
			String strUpdateSMSMOStatus = " update esme_sms_mo set status = ?, last_update=sysdate where mo_id = ?  ";
	...
	public int updateSMSLogStatus(Connection mcnMain, String status,
		...
		try {
			...
			strUpdateSMSLogStatus += " ,last_update=sysdate where log_id =? ";
	...
	public int updateSMSLogTranID(Connection mcnMain, String status,
		...
		try {
			...
			String strUpdateSMSLogTranid = " update esme_sms_log set status =?,SMSC_DELIVERY_ID=?,ack_date = sysdate,last_update=sysdate where ack_id =? ";
	...
	public int updateSMSRetryStatus(Connection mcnMain, int status,
		...
		try {
			String strSqlUpdateRetryStatus = " UPDATE ESME_SMS_MT SET STATUS = ? ,sms_log_id=?, RETRY_NUMBER = nvl(RETRY_NUMBER,0)+1 ,  LAST_RETRY_TIME = SYSDATE WHERE MT_ID = ? ";
	...
	public int updateSMSReloadStatus(Connection mcnMain, String status,
		...
		try {
			String strSqlUpdateReloadStatus = " UPDATE ESME_SMS_MT SET STATUS = ? , RELOAD_NUMBER = nvl(RETRY_NUMBER,0)+1 ,  LAST_RETRY_TIME = SYSDATE WHERE MT_ID = ? ";
	 ...
	 * @throws Exception
	 */
	public void insertSMSLog(Connection mcnMain, String smslogId,
			String request_id, String msisdn, String status, String type,
			String response_id, String service_id, String service_parent_id,
			...
			String subId, String groupID,int customerid,int advid,int campaignid) throws Exception {
		...
		try {
			...
			pstmtInsertSMSLog.executeQuery();
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw e;
	...
	public int insertSMSMT(Connection mcnMain, String requestid,
		...
		try {
			String strSqlCommand = " insert into ESME_SMS_MT(MT_ID,REQUEST_ID,REQUEST_TIME,MESSAGE,CP_ID,SHORT_CODE,MSISDN,RETRY_NUMBER,COMMAND_CODE,REGISTER_DELIVERY_REPORT,STATUS, ESME_TRANS_ID, MO_SEQUENCE_NUMBER,sub_id,group_id)"
					+ " values ( sms_mt_seq.nextval,?,sysdate,?, ?,?,?,'0',?,?,'0', ?, ?,?,?) ";
			pstmInsertMT = mcnMain.prepareStatement(strSqlCommand);
			pstmInsertMT.setString(1, requestid);
			pstmInsertMT.setString(2, message);
			pstmInsertMT.setString(3, cp_id);
			pstmInsertMT.setString(4, shortcode);
			pstmInsertMT.setString(5, msisdn);
			pstmInsertMT.setString(6, commandcode);
			pstmInsertMT.setString(7, registerDl);
			pstmInsertMT.setString(8, strESMETransID);
			pstmInsertMT.setString(9, strMOSequenceNumber);
			pstmInsertMT.setString(10, subid);
			pstmInsertMT.setString(11, groupuid);
	...
	public int insertSMSMTEx(Connection mcnMain, String requestid,
			String message, String cp_id, String shortcode, String msisdn,
			String commandcode, String registerDl, String strESMETransID,
			String strMOSequenceNumber, String subid, String groupuid,int customerId,int campaign_id,int adv_id) {
		PreparedStatement pstmInsertMT = null;
		try {
			String strSqlCommand = " insert into ESME_SMS_MT(MT_ID,REQUEST_ID,REQUEST_TIME,MESSAGE,CP_ID,SHORT_CODE,MSISDN,RETRY_NUMBER,COMMAND_CODE,REGISTER_DELIVERY_REPORT,STATUS, ESME_TRANS_ID, MO_SEQUENCE_NUMBER,SUB_ID,GROUP_ID,CUSTOMER_ID,CAMPAIGN_ID, ADV_ID)"
					+ " values ( sms_mt_seq.nextval,?,sysdate,?, ?,?,?,'0',?,?,'0', ?,?,?,?,?,?,?) ";
			pstmInsertMT = mcnMain.prepareStatement(strSqlCommand);
			pstmInsertMT.setString(1, requestid);
			pstmInsertMT.setString(2, message);
			...
			pstmInsertMT.setString(10, subid);
			pstmInsertMT.setString(11, groupuid);
			pstmInsertMT.setInt(12, customerId);
			pstmInsertMT.setInt(13, campaign_id);
			pstmInsertMT.setInt(14, adv_id);
			return pstmInsertMT.executeUpdate();
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
	...
	public int updateMTForMO(Connection mcnMain, String strResponseID,
			String strLogID, String strMSISDN) throws Exception {
