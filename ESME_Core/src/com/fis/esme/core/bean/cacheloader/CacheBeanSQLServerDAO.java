...
public abstract class CacheBeanSQLServerDAO extends CacheBean {
	...
	public  List<SMSCMTRountingObject> loadSMSMTRouting(Connection con) {
		...
		try {
			String strSql = " select smsc.smsc_id, smsc.code,smsc.defaul_short_code,smsc.defaul_short_code,isdn.prefix_value,ISNULL(REPLACE(REPLACE(prefix_value,'*',''),'?',''),'0') "
					+ " order_value,len(ISNULL(REPLACE(REPLACE(prefix_value,'*',''),'?',''),'0')) priority from esme_smsc smsc,esme_isdn_prefix isdn,esme_smsc_routing routing "
	...
	public  List<SMSMORoutingInfo> SMSRouting(Connection con) {
		...
		try {
			...
			String strSql = " select sv.service_id,sv.root_id,sc.short_code_id,sc.code,cmd.command_id,cmd.code CMD_CODE,cp.cp_id ,cp.code CP_CODE,cp.protocal,sv.parent_id,cp.receive_url_msg,cp.receive_username,cp.receive_password,ISNULL(REPLACE(REPLACE(cmd.code,'*',''),'?',''),'0') cmd_order "
	 ...
	 * @param smsMO
	 */
	private static List<SMSMORoutingInfo> addSMSMORouting(
	...
	private static int formatInt(String input) {
		if (input == null || input == "") {
			return 0;
		} else {
			try {
