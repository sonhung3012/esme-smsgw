...
public class SMSTransceiverEx4Test extends ManageableThreadEx {
	String syncMode;
	/**
	...
	private String mstrDeliveryStatusValue = "DELIVRD";
	/**
	 * The debug object.
	 */
 ...
 */
						...
						if (Text_SMS.length() > iMaxLengthSingleMessage) {
							...
							for (int i = 0; i < totalSegments; i++) {
								...
								if (isReceiveDelivery()) {
									smRequest.setRegisteredDelivery((byte) 1);
								} else {
									smRequest.setRegisteredDelivery((byte) 0);
								}
								smRequest
							...
							if (isReceiveDelivery()) {
								submitRequest.setRegisteredDelivery((byte) 1);
							} else {
	...
	private void enquireLink() throws Exception {
		...
		String strDebug = "";
		logMonitor("Enquire Link pre request " + request.debugString());
		if (asynchronous) {
	...
	private boolean receive() throws Exception {
		...
		if (pdu.isRequest()) {
			...
			if (pdu instanceof DeliverSM) {
				...
				if (isReceiveDelivery() && esmClass == DELIVERY_REPORT_SIGNAL) // tong
																				// dai
																				// SMSC
																				// thong
																				// bao
															...
															// dai 9232
				{
					...
					receiveMessage.setAttribute("request_id",String.valueOf(deliverSM.getSequenceNumber()));
			...
			} else // Ack thue bao den he thong 9232
			{
				try {
					...
					//mqlSMSCQueueManager.processResponse(pdu);
				} catch (Exception e) {
					// TODO: handle exception
					e.printStackTrace();
